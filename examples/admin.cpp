...
int main(int argc, char** argv) {
    ...
    try {
        ...
        if(g_operation == "create") {
            auto id = admin.createCache(g_address, g_provider_id,
                g_type, g_config, g_token);
            spdlog::info("Created cache {}", id.to_string());
        } else if(g_operation == "open") {
            auto id = admin.openCache(g_address, g_provider_id,
                g_type, g_config, g_token);
            spdlog::info("Opened cache {}", id.to_string());
        } else if(g_operation == "close") {
            admin.closeCache(g_address, g_provider_id,
                cachersize::UUID::from_string(g_cache.c_str()), g_token);
            spdlog::info("Closed cache {}", g_cache);
        } else if(g_operation == "destroy") {
            admin.destroyCache(g_address, g_provider_id,
                cachersize::UUID::from_string(g_cache.c_str()), g_token);
            spdlog::info("Destroyed cache {}", g_cache);
        }

        // Any of the above functions may throw a cachersize::Exception
...
void parse_command_line(int argc, char** argv) {
    try {
        TCLAP::CmdLine cmd("Cachersize admin", ' ', "0.1");
        ...
        TCLAP::ValueArg<std::string> typeArg("t","type","Cache type", false,"dummy","string");
        TCLAP::ValueArg<std::string> cacheArg("r","cache","Cache id", false, cachersize::UUID().to_string(),"string");
        TCLAP::ValueArg<std::string> configArg("c","config","Cache configuration", false,"","string");
